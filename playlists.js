(() => {
  ...
  if (!state.deviceId || !state.deviceKey) { window.location.href = "../"; return; }

  const $ = (id) => document.getElementById(id);
  $("deviceLabel").textContent = state.deviceId;
  $("deviceKeyLabel").textContent = state.deviceKey;
  ...
  async function loadPlaylists() {
    const query = new URLSearchParams({ deviceId: state.deviceId, deviceKey: state.deviceKey });
      ...
      remove.addEventListener("click", async () => {
        ...
        try {
          await request("/api/playlists/delete", {
            method: "POST",
            body: JSON.stringify({ deviceId: state.deviceId, deviceKey: state.deviceKey, id: playlist.id })
          });
  ...
  form.addEventListener("submit", async (event) => {
    ...
    try {
      await request("/api/playlists", {
        method: "POST",
        body: JSON.stringify({ deviceId: state.deviceId, deviceKey: state.deviceKey, name, server, username, password, hidden: $("hiddenPlaylist").checked, locked: $("lockedPlaylist").checked })
      });
